...
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;600;700&family=Syne:wght@700;800&family=IBM+Plex+Sans:wght@300;400;600;700&display=swap" rel="stylesheet">
<style>
...
@keyframes glow { 0%,100%{box-shadow:0 0 20px #6699ff33} 50%{box-shadow:0 0 60px #6699ff66} }
...
* { box-sizing: border-box; margin: 0; padding: 0; }
body { background: #07070f; color: #d8daf0; font-family: 'IBM Plex Mono', monospace; min-height: 100vh; }
@keyframes spin { to { transform: rotate(360deg); } }
...
<section class="section" id="features">
  ...
  <div class="features-grid">
    <div class="feature-card fade-up delay-1">
      ...
      <div class="feature-title">REAL-TIME GOOGLE DATA</div>
      <div class="feature-desc">Every search pulls live data directly from Google Places. Real businesses, real addresses, real phone numbers — updated constantly.</div>
    </div>
    ...
    <div class="feature-card fade-up delay-1">
      ...
      <div class="feature-desc">Instant links to CourtListener, PACER, and Google lawsuit search. Know before you call if a business has existing MCA debt.</div>
    </div>
    <div class="feature-card fade-up delay-2">
      <span class="feature-icon">🎯</span>
      <div class="feature-title">13 INDUSTRY VERTICALS</div>
      <div class="feature-desc">Trades, healthcare, food & franchise — target the exact business types most likely to need working capital.</div>
    </div>
    <div class="feature-card fade-up delay-3">
      <span class="feature-icon">📥</span>
  ...
  <div class="how-inner">
    ...
    <div class="steps">
      ...
      <div class="step fade-up delay-2">
        ...
        <div class="step-desc">Google Places returns real verified businesses in seconds.</div>
      </div>
...
<section class="pricing-section" id="pricing">
  ...
  <div class="pricing-grid" style="grid-template-columns:1fr;max-width:520px;margin:0 auto">
...
<section class="faq-section" id="faq">
  ...
  <div class="faq-item">
    <div class="faq-q">What counts as one search?</div>
    <div class="faq-a">Each individual lead returned counts as one search. If you search for 25 leads, that uses 25 searches from your monthly total. This keeps your usage transparent and predictable — you always know exactly what you're spending.</div>
  </div>
  <div class="faq-item">
  ...
  <div class="faq-item">
    ...
    <div class="faq-a">Trades (roofing, HVAC, electrical, restoration), Food & Franchise (restaurants, catering, ghost kitchens, franchises), and Healthcare (dental, med spas, urgent care, physical therapy, behavioral health).</div>
  </div>
  <div class="faq-item">
...
const SEARCH_TERMS = {
  "Commercial Roofing":              ["commercial roofing contractor", "roofing company", "roof repair contractor", "roofer", "roofing installation", "flat roof contractor", "metal roofing company", "residential roofing"],
  "HVAC / Plumbing":                 ["HVAC contractor", "plumbing contractor", "air conditioning company", "heating cooling company", "AC repair company", "plumber", "HVAC repair service", "ductwork contractor", "mechanical contractor"],
  "Electrical Contractors":          ["electrical contractor", "electrician company", "commercial electrician", "electrical services company", "licensed electrician", "industrial electrician", "electrical installations"],
  "Restoration & Remediation":       ["water damage restoration", "mold remediation company", "fire restoration contractor", "flood damage repair", "disaster restoration", "smoke damage restoration", "sewage cleanup company"],
  "Restaurant Groups (2+ locations)":["restaurant group", "food service company", "multi-location restaurant", "restaurant chain", "food and beverage group", "bar and grill", "hospitality group", "dining group"],
  "High-volume Catering":            ["catering company", "corporate catering", "event catering service", "wedding catering", "full service catering", "catering business", "banquet catering"],
  "Ghost Kitchen Operators":         ["ghost kitchen", "virtual restaurant kitchen", "cloud kitchen", "delivery only restaurant", "virtual kitchen", "dark kitchen"],
  "Multi-unit Franchise":            ["franchise business", "franchise owner", "multi-unit franchise", "franchise operator", "franchise group", "independent franchise"],
  "Dental Practice":                 ["dental office", "dental practice", "dentist clinic", "family dentistry", "cosmetic dentistry", "dental group", "oral health clinic", "dental center", "periodontist", "orthodontist office"],
  "Med Spa":                         ["med spa", "medical spa", "aesthetic clinic", "medspa", "laser clinic", "skin care clinic", "botox clinic", "aesthetic center", "wellness spa"],
  "Urgent Care Clinic":              ["urgent care clinic", "walk in clinic", "immediate care center", "urgent care center", "emergency care clinic", "after hours clinic", "quick care clinic"],
  "Physical Therapy":                ["physical therapy clinic", "physical therapist", "sports rehabilitation", "PT clinic", "orthopedic rehab", "sports medicine clinic", "rehabilitation center", "occupational therapy"],
  "Behavioral Health":               ["behavioral health clinic", "mental health practice", "counseling center", "therapy clinic", "psychotherapy office", "outpatient mental health", "substance abuse treatment", "addiction treatment center"],
};
...
const INDUSTRY_GROUPS = {
  "🏗️ Trades": ["Commercial Roofing","HVAC / Plumbing","Electrical Contractors","Restoration & Remediation"],
  "🍽️ Food & Franchise": ["Restaurant Groups (2+ locations)","High-volume Catering","Ghost Kitchen Operators","Multi-unit Franchise"],
  "🏥 Healthcare": ["Dental Practice","Med Spa","Urgent Care Clinic","Physical Therapy","Behavioral Health"],
};
...
let state = {
  // Auth
  authView: authToken ? 'loading' : 'login', // 'login' | 'signup' | 'loading' | 'app'
  authError: '',
  authLoading: false,
  // App
  leads: [],
...
(async () => {
  if (authToken) {
    ...
    setState({authView: ok ? 'app' : 'login'});
    warmSBA(); // start warming immediately when returning user loads app
  }
})();
...
async function fetchLeads() {
  ...
  setState({loading: true, leads: [], errorMsg: '', progress: 10, loadingMsg: '🔍 Searching Google Places...'});

  const city = state.selectedCity.trim();
  const st = state.selectedState;
  const location = city ? `${city}, ${st}` : (st === 'All States' ? 'United States' : st);

  try {
    // Each industry runs all its search term variants + nearby suburbs in parallel
    const perTerm = 20;
    const nearbyAreas = getNearbyAreas(city);
    // Use primary location + up to 3 nearby areas for diversity
    const searchLocations = [location, ...nearbyAreas.slice(0, 3).map(a => `${a}, ${st}`)];

    const searches = state.selectedIndustries.flatMap(ind => {
      const terms = SEARCH_TERMS[ind];
      const termList = Array.isArray(terms) ? terms : [terms || ind];
      // Primary location uses all terms; nearby areas use first 2 terms only (avoid rate limits)
      return searchLocations.flatMap((loc, locIdx) => {
        const termsForLoc = locIdx === 0 ? termList : termList.slice(0, 2);
        return termsForLoc.map(term => {
          const url = `${PROXY_URL}/search?type=${encodeURIComponent(term)}&location=${encodeURIComponent(loc)}&limit=${perTerm}`;
          return fetch(url, {headers: {'Authorization': 'Bearer ' + authToken}})
            ...
            .catch(e => { console.warn('Search failed:', term, loc, e.message); return []; });
        });
      });
    });
    ...
    if (state.sbaOnly) {
      ...
      if (b.officerName) links.truePeopleName = `https://www.truepeoplesearch.com/results?name=${encodeURIComponent(b.officerName)}&citystatezip=${encodeURIComponent(bState)}`;
            const lead = {...b, id: `lead-${i}-${Date.now()}`, city: bCity, state: bState,
              ...
              bizType: null, dataSource: 'Google Places', verified: true, enriched: false,
    ...
    if (flat.length === 0) {
      setState({errorMsg: 'No businesses found. Try a different city.', loading: false});
      return;
    }
  ...
  <div class="lead-card ${expanded ? 'expanded' : ''}" id="card-${lead.id}">
    <div style="padding:14px 16px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
        <div style="flex:1">
          ...
          <div style="display:flex;gap:5px;flex-wrap:wrap">
            ...
            ${lead.officerName ? `<span class="tag" style="background:#0d0a1a;color:#aa88ff;border:1px solid #2a1a4a">👤 ${escHtml(lead.officerName.split(' ').slice(0,3).join(' '))}</span>` : ''}
        ...
        <div style="display:flex;gap:6px;margin-left:12px;flex-shrink:0">
          ...
          ${!isSaved ? (() => {
            ...
            const tip = tpPhone ? 'Reverse lookup business phone — may be owner cell' : tpName ? `Search owner by name: ${lead.officerName}` : 'Enrich first to get phone or officer name';
      ...
      <div style="background:#080818;border:1px solid #1a0f30;border-radius:8px;padding:10px 14px;margin-bottom:8px;display:flex;flex-wrap:wrap;gap:10px;align-items:center">
        ...
        ${lead.officerName ? '<span style="font-size:12px;color:#cc99ff;font-weight:600">' + escHtml(lead.officerName) + '</span>' : ''}
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px 16px;font-size:11px">
        ...
        ${lead.phone ? `<div><span style="color:#3a3a6a">📞 </span><span style="color:#aabbff;font-weight:600">${escHtml(lead.phone)}</span></div>` : ''}
        ${lead.website ? `<div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><span style="color:#3a3a6a">🌐 </span><a href="${safeUrl(lead.website)}" target="_blank" style="color:#4488ff">${escHtml((lead.website||'').replace(/^https?:\/\/(www\.)?/,'').split('/')[0])}</a></div>` : ''}
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px">
        <div style="background:#0a0a1e;border:1px solid #181835;border-radius:8px;padding:12px">
          ...
          <div style="font-size:11px;color:#6a6a9a;line-height:2">
            ...
            ${lead.officerName ? `<div><span style="color:#4a4a7a">Officer: </span><span style="color:#aa88ff;font-weight:600">${escHtml(lead.officerName)}</span></div>` : ''}
        ...
        <div style="background:#080f08;border:1px solid #0a2a0a;border-radius:8px;padding:12px">
          ...
          <div style="display:flex;flex-direction:column;gap:6px">
            ${lead.officerName ? `<div style="font-size:12px;color:#cc99ff;font-weight:700;padding:4px 0">👤 ${escHtml(lead.officerName)}</div>` : ''}

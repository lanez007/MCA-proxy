...
// All users get unlimited searches
const PLAN_LIMIT = Infinity;
...
async function initDB() {
  ...
  // Google Details cache — pay once per place_id, use forever
  // At 5k searches/month reaches ~60% hit rate after first week = huge savings
  await pool.query(`
    CREATE TABLE IF NOT EXISTS google_details_cache (
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/" || url.pathname === "/warmup") {
    ...
    try {
      const cacheStats = await pool.query('SELECT COUNT(*) as total FROM google_details_cache');
      const cacheCount = parseInt(cacheStats.rows[0]?.total || 0);
      respond(res, 200, { status: "ok", service: "RCN Lead Gen API", sba_ready: !!sbaIndex, sba_loading: sbaLoading, sba_error: sbaLoadError || null, sba_count: sbaIndex ? Object.keys(sbaIndex).length : 0, google_cache_size: cacheCount });
    } catch(_) {
      respond(res, 200, { status: "ok", service: "RCN Lead Gen API", sba_ready: !!sbaIndex });
    }
    return;
  }
  ...
  if (url.pathname === "/auth/register" && req.method === "POST") {
    ...
    try {
      ...
      const result = await pool.query(
        "INSERT INTO users (email, password_hash, plan, referral_code, referred_by) VALUES ($1, $2, 'unlimited', $3, $4) RETURNING id, email, plan, searches_used, referral_code, name, is_admin",
    ...
    try {
      ...
      respond(res, 200, {
        ...
        user: { id: user.id, email: user.email, name: user.name, plan: user.plan, searches_used: user.searches_used,
          searches_remaining: null, limit: null, referral_code: user.referral_code,
  ...
  if (url.pathname === "/auth/me" && req.method === "GET") {
    ...
    try {
      ...
      respond(res, 200, { user: { id: user.id, email: user.email, name: user.name, plan: user.plan,
        ...
        searches_remaining: null, limit: null, referral_code: user.referral_code, referred_by: user.referred_by,
  ...
  // SEARCH — uses HERE (primary, 250k free/mo) + Yelp (secondary, 500 free/day)
  // + OpenStreetMap Overpass (tertiary, free unlimited) as fallbacks
  if (url.pathname === "/search") {
    ...
    try {
      ...
      // Unlimited plan — no search cap
      console.log(`[SEARCH] type="${type}" location="${location}" limit=${limit}`);

      // ── STEP 1: GEOCODE — Nominatim (OpenStreetMap, free, no key needed) ──
      let lat = null, lng = null;
      try {
        [lat, lng] = await geocodeNominatim(location);
      } catch(_) {}

      if (lat === null || lat === undefined) {
        respond(res, 404, { error: `Location not found: "${location}". Try "Miami, FL" or "Houston, TX".` });
        return;
      }
      console.log(`[SEARCH] Geocoded "${location}" → ${lat},${lng}`);

      // ── STEP 2: FETCH BUSINESSES — all sources in parallel ──────────────────
      const [hereResults, googleResults, osmResults] = await Promise.all([
        // HERE: 250k free calls/month — best structured data
        hereSearch(type, lat, lng, limit),
        // Google Places: best phone/rating/status data (~$0.017/req, key optional)
        GOOGLE_API_KEY ? googlePlacesSearch(type, lat, lng, limit) : Promise.resolve([]),
        // OSM: completely free, always runs
        osmSearch(type, lat, lng, limit),
      ]);
      console.log(`[SEARCH] Raw: here=${hereResults.length} google=${googleResults.length} osm=${osmResults.length}`);
      ...
      const merged    = [];
      const addResult = (r) => {
        const idKey   = r.placeId || '';
        const nameKey = (r.businessName || '').toLowerCase().replace(/[^a-z0-9]/g, '').substring(0, 25);
      ...
      // Priority: HERE first (best data), then OSM
      hereResults.forEach(addResult);
      googleResults.forEach(addResult);
      osmResults.forEach(addResult);

      console.log(`[SEARCH] Merged: ${merged.length} unique businesses`);

      if (merged.length === 0) {
        respond(res, 200, { results: [], debug: `Zero results — HERE returned nothing. here_key=${!!HERE_API_KEY} google_key=${!!GOOGLE_API_KEY}. Check Railway env vars.`, searches_used: user.searches_used, searches_remaining: planLimit === Infinity ? null : planLimit - user.searches_used, limit: planLimit === Infinity ? null : planLimit });
        return;
      }

      const detailed = merged.slice(0, limit);

      // ── STEP 4: AUTO-SOS owner lookup ───────────────────────────────────────
      ...
      await Promise.all(detailed.map(async (lead) => {
        try {
          ...
          if (!sosData?.officerName || sosData?.officerType === 'agent') {
            ...
            if (dirResult?.officerName) sosData = { ...(sosData || {}), officerName: dirResult.officerName, officerType: 'owner' };
          }
          if (sosData?.officerName && !cachedSOS) cacheOwner(lead.businessName, stateCode, sosData).catch(()=>{});
          if (sosData) {
      ...
      await pool.query("UPDATE users SET searches_used = searches_used + $1 WHERE id = $2", [detailed.length, user.id]);
      const newUsed = user.searches_used + detailed.length;
    ...
    } catch(err) { console.error('[SEARCH] Error:', err.message); respond(res, 500, { error: err.message }); }
    return;
  }
  ...
  if (url.pathname === "/enrich") {
    ...
    try {
      ...
      let phone = null, siteUrl = websiteHint || null, address = null, businessStatus = null;
      // websiteHint from search result — enrich will scrape it below for email/phone
      if (websiteHint) siteUrl = websiteHint;

      // Google Places Details — fill phone/website for HERE & OSM leads that lack them
      // Google leads already got phone/website at search time (called Details in parallel)
      // so only call Details here for here_ and osm_ leads that are missing data
      const needsDetails = GOOGLE_API_KEY && placeId && (placeId.startsWith('here_') || placeId.startsWith('osm_')) && (!siteUrl);
      if (needsDetails) {
        try {
          // Use text search to find the Google place for this HERE/OSM business
          const q = encodeURIComponent(bizName + ' ' + bizState);
          const searchUrl = `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${q}&key=${GOOGLE_API_KEY}`;
          const searchRaw = await fetchURL(searchUrl, 5000);
          const searchData = JSON.parse(searchRaw);
          const googlePlaceId = searchData.results?.[0]?.place_id;
          if (googlePlaceId) {
            const d = await googlePlaceDetails(googlePlaceId);
            if (d) {
              phone          = d.formatted_phone_number || phone  || null;
              siteUrl        = d.website                || siteUrl || null;
                          businessStatus = d.business_status        || null;
                          console.log(`[ENRICH] Google Details (HERE/OSM fallback) → phone=${phone?'YES':'NO'} | website=${siteUrl?'YES':'NO'}`);
            }
          }
        } catch(_) {}
      }
      ...
      const cachedEmail  = cachedResult?.email || null;
      const cacheSource  = cachedResult?.source || null;

      // If nothing found yet, try permutation estimate from officer name
      const permResult   = (!cachedEmail && !ownerEmail && !hunterEmail && !scrapedEmail && officerName && domain)
        ? bestPermutationGuess(officerName, domain)
        : null;

      const bestEmail = ownerEmail || cachedEmail || hunterEmail || scrapedEmail || permResult?.email || null;
      const emailSource = ownerEmail ? 'pdl-personal'
        : cachedEmail ? cacheSource
        : hunterEmail ? 'hunter'
      ...
      if (domain && bestEmail && !cachedEmail) {
  ...
  if (url.pathname === "/profile" && req.method === "PUT") {
    ...
    try {
      ...
      const updated = await getUser(user.id);
            respond(res, 200, { message: "Profile updated", user: { id: updated.id, email: updated.email, name: updated.name, plan: updated.plan, searches_used: updated.searches_used, searches_remaining: null, limit: null, referral_code: updated.referral_code, plan_start_date: updated.plan_start_date, is_admin: updated.is_admin } });
  ...
  if (url.pathname.startsWith("/admin/users/") && url.pathname.endsWith("/plan") && req.method === "PUT") {
    ...
    if (plan !== "unlimited") { respond(res, 400, { error: "Invalid plan" }); return; }
    try {
...
function buildSOSUrl(bizName, state) {
  ...
  return urls[state] || `https://www.bing.com/search?q=${encodeURIComponent(bizName + " " + state + " registered agent owner")}`;
}
...
// Scrapes business website + public directories (BBB, Manta, Whitepages)
...
// Rate limit: 1 req/sec — acceptable for our use (one geocode per search)
async function geocodeNominatim(locationStr) {
  try {
    // Add featuretype hint — helps Nominatim find states and cities faster
    const isStateOnly = /^[A-Za-z\s]+$/.test(locationStr.trim()) && !locationStr.includes(',');
    const featureHint = isStateOnly ? '&featuretype=state' : '';
    const nomPath = `/search?q=${encodeURIComponent(locationStr)}&format=json&limit=3&countrycodes=us&addressdetails=1${featureHint}`;
    const raw = await new Promise((resolve, reject) => {
      const req = https.request({ hostname: 'nominatim.openstreetmap.org', path: nomPath, method: 'GET',
        headers: { 'User-Agent': 'RCNLeadGen/1.0 (contact@thercngroup.com)', 'Accept': 'application/json' }
      }, (res) => {
        let data = '';
      ...
      req.setTimeout(5000, () => { req.destroy(); reject(new Error('Nominatim timeout')); });
      req.on('error', reject);
      req.end();
    });
    ...
    // Prefer results that are states, cities, or towns (not just address matches)
    const best = results.find(r => ['state','city','town','village','county'].includes(r.type) || r.class === 'boundary')
                 || results[0];
    const lat = parseFloat(best.lat);
    const lng = parseFloat(best.lon);
    console.log(`[GEO] Nominatim "${locationStr}" → ${lat},${lng} (${best.display_name?.split(',')[0]})`);
    return [lat, lng];
  } catch(e) {
    console.log(`[GEO] Nominatim failed: ${e.message}`);
    return [null, null];
  }
}

// ─── SEARCH: OpenStreetMap Overpass (free, no key, always works) ─
// Converts business type to OSM tags and queries within bounding box
async function osmSearch(type, lat, lng, limit) {
  try {
    // Map search terms to OSM amenity/shop tags
    const OSM_TAGS = {
      'roofing':           [['craft','roofer'], ['shop','roofer']],
      'hvac':              [['shop','hvac'], ['craft','hvac']],
      'plumb':             [['craft','plumber'], ['shop','plumber']],
      'electric':          [['craft','electrician'], ['shop','electrician']],
      'dental':            [['amenity','dentist']],
      'dentist':           [['amenity','dentist']],
      'restaurant':        [['amenity','restaurant']],
      'cafe':              [['amenity','cafe']],
      'auto':              [['shop','car_repair'], ['amenity','car_repair']],
    ...
    let tagFilters = null;
    for (const [kw, tags] of Object.entries(OSM_TAGS)) {
      if (typeLow.includes(kw)) { tagFilters = tags; break; }
    }
    if (!tagFilters) return []; // OSM doesn't have a generic business search

    // ~25km bounding box around lat/lng
    const delta = 0.22;
    const bbox  = `${lat-delta},${lng-delta},${lat+delta},${lng+delta}`;
    const tagQueries = tagFilters.map(([k,v]) => `node["${k}"="${v}"](${bbox});way["${k}"="${v}"](${bbox});`).join('');
    const query  = `[out:json][timeout:10];(${tagQueries});out center ${limit * 3};`;
    const url    = `https://overpass-api.de/api/interpreter?data=${encodeURIComponent(query)}`;
    const raw    = await fetchURL(url, 12000, 300000);
    if (!raw) return [];
    const data   = JSON.parse(raw);
    const items  = (data.elements || []).filter(e => e.tags?.name);
    console.log(`[OSM] returned ${items.length} named elements for "${type}"`);
    return items.slice(0, limit).map(e => {
      ...
      return {
        ...
        address: [tags['addr:housenumber'], tags['addr:street'], tags['addr:city'], tags['addr:state'], tags['addr:postcode']].filter(Boolean).join(', ') || `${elat?.toFixed(4)}, ${elng?.toFixed(4)}`,
  ...
  } catch(e) { console.error('[OSM] Error:', e.message); return []; }
}
...
// Only runs if GOOGLE_API_KEY is set. ~$0.017/request.
// Hard cap enforced via Google Cloud budget alarm ($100/mo recommended).
// ─── GOOGLE PLACES DETAILS CACHE ─────────────────────────────────
// Caches phone+website by place_id in DB — never pay for the same place twice
// At 5,000 searches/month, cache hit rate reaches ~60% after first week
async function getGoogleDetailsCache(placeId) {
  try {
    const r = await pool.query('SELECT phone, website, business_status FROM google_details_cache WHERE place_id=$1', [placeId]);
    return r.rows[0] || null;
  } catch(_) { return null; }
}
async function setGoogleDetailsCache(placeId, phone, website, businessStatus) {
  try {
    await pool.query(
      `INSERT INTO google_details_cache (place_id, phone, website, business_status, fetched_at)
       VALUES ($1,$2,$3,$4,NOW())
       ON CONFLICT (place_id) DO UPDATE SET phone=$2, website=$3, business_status=$4, fetched_at=NOW()`,
      [placeId, phone||null, website||null, businessStatus||null]
    );
  } catch(_) {}
}

// Fetch Google Place Details using NEW Places API v1
// Contact fields only = $3.00/1k (vs $13.60/1k on old API) — 78% cheaper
// Checks DB cache first — cache hit = FREE
async function googlePlaceDetailsV2(placeId) {
  if (!GOOGLE_API_KEY || !placeId) return null;

  // 1. Check cache first — completely free
  const cached = await getGoogleDetailsCache(placeId);
  if (cached) return cached;

  // 2. Cache miss — call new Places API v1 (contact fields only)
  try {
    const url = `https://places.googleapis.com/v1/places/${placeId}?fields=nationalPhoneNumber,websiteUri,businessStatus&key=${GOOGLE_API_KEY}`;
    ...
    const businessStatus = d.businessStatus || null;

    // Store in cache for future searches — pay once, use forever
    await setGoogleDetailsCache(placeId, phone, website, businessStatus);
    return { phone, website, business_status: businessStatus };
  } catch(_) { return null; }
}

async function googlePlacesSearch(term, lat, lng, limit) {
  if (!GOOGLE_API_KEY) return [];
  try {
    // Step 1: Text Search (old API) — gets place_ids + business_status
    // Using old textsearch intentionally: $25.60/1k vs new API $35/1k
    const q   = encodeURIComponent(term);
    const url = `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${q}&location=${lat},${lng}&radius=40000&key=${GOOGLE_API_KEY}`;
    const raw = await fetchURL(url, 7000);
    const data = JSON.parse(raw);
    if (!data.results?.length) return [];

    // Page 2 for more results (Google max 20/page)
    let candidates = data.results || [];
    if (limit > 20 && data.next_page_token) {
      try {
        await new Promise(r => setTimeout(r, 1500));
        const p2Url = `https://maps.googleapis.com/maps/api/place/textsearch/json?pagetoken=${data.next_page_token}&key=${GOOGLE_API_KEY}`;
        const p2Raw = await fetchURL(p2Url, 7000);
        const p2    = JSON.parse(p2Raw);
        candidates  = [...candidates, ...(p2.results || [])];
      } catch(_) {}
    }
    candidates = candidates.slice(0, Math.min(limit, 60));

    // Step 2: Details in parallel — NEW API v1 with DB cache
    // COST CONTROL: only call Details if HERE didn't already give us a phone
    // This function handles caching automatically — cache hit = $0.00
    const details = await Promise.all(
      candidates.map(p => googlePlaceDetailsV2(p.place_id))
    );
    ...
    return candidates.map((p, i) => {
      ...
      return {
        ...
        phone:          d?.phone             || d?.formatted_phone_number || null,
        website:        d?.website           || null,
        placeId:        p.place_id,
        businessStatus: d?.business_status   || p.business_status || null,
        source:         'google',
      };
    });
  } catch(_) { return []; }
...
// Legacy wrapper — used in enrich for HERE/OSM leads
// Also uses cache + new API v1
async function googlePlaceDetails(placeId) {
  if (!GOOGLE_API_KEY || !placeId || placeId.startsWith('here_') || placeId.startsWith('osm_')) return null;
  const result = await googlePlaceDetailsV2(placeId);
  if (!result) return null;
  // Map back to old field names for enrich compatibility
  return {
    formatted_phone_number: result.phone || null,

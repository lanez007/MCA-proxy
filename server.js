      ...
      const googlePagesPromise = (async () => {
        for (let page = 0; page < 3 && allPlaces.length < limit; page++) {
          ...
          const results = (placesData.results || []).filter(p => {
            ...
            // Google Places uses "United States" not "USA" for most results — accept both
            if (addr.includes('UNITED STATES') || addr.includes(', USA') || addr.endsWith(' USA')) return true;
            if (/,\s*[A-Z]{2}\s+\d{5}/.test(addr)) return true; // "FL 33101" zip pattern
            // Reject known non-US countries
            if (addr.includes('CANADA') || addr.includes('MEXICO') || addr.includes('UNITED KINGDOM') || addr.includes('AUSTRALIA')) return false;
            return true; // geo-bounded search — default keep
          });
